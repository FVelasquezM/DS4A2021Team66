...
```bash
predict-app$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
predict-app$ sam local start-api
predict-app$ curl http://localhost:3000/classify_digit
```
...
```bash
predict-app$ sam logs -n InferenceFunction --stack-name predict-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name predict-app
```
